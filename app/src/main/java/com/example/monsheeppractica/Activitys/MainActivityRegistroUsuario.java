...
public class MainActivityRegistroUsuario extends AppCompatActivity {
    ...
    DatabaseHandler db;
    ImageView iv_foto;
    String idClienteExtra,editar_extra,tipoTelefonoExtra,NombreExtra,CalleExtra,NumeroExtra,InteriorExtra
            ,CodigoPostalExtra,ColoniaExtra,MunicipioExtra,EstadoExtra,AliasExtra,LadaExtra,TeléfonoExtra
            ,ApellidoPaternoExtra,ApellidoMaternoExtra,TipoCompraExtra,Id_Foto_extra;
    ...
    private void ValidarExistenciaUsuario() {
        ...
        if (bh != null) {
            ...
            SQLiteDatabase db = bh.getReadableDatabase();
            Cursor c = db.rawQuery("select * from clientes where Teléfono=='"+etTelefono.getText().toString().trim()+"' and status='Activo'", null);
            if (c.moveToFirst()) {
//                do {
...
//                } while (c.moveToNext());
    ...
    public void Validar_datos(){
        ...
        try {
                                    ...
                                    if (!Estado.isEmpty() && Estado.length() >= 5){//Son muy pocos caracteres para una colonia
                                        if (!Lada.isEmpty() && Lada.length() >= 2 && Lada.length() <=3){//Son mu
                                            if (!Teléfono.isEmpty() && Teléfono.length() == 10){//Son mu
                                                ...
                                                }else etRepContra.setError("Las contraseñas no coinciden");
                                            }else  etTelefono.setError("El campo Teléfono es obligatoria y debe ser de 10 caracteres");
                                        }else  etLada.setError("El campo Lada es obligatoria y debe ser al menos de 2 caracteres");
            ...
            }else etNombre.setError("El campo nombre es obligatorio y debe ser al menos de 5 caracteres");
        }catch (Exception e){
            Toast.makeText(getApplicationContext(), ""+e, Toast.LENGTH_SHORT).show();
            System.out.println(""+e);
        }
